...
class ParamsEditor(http.Controller):
    ...
    def editor_get(self, osv_info, editor_id, ):
        ...
        return request.make_response(json.dumps(
            ...
            } for row in getattr(request.env[osv_info.get('model', 'maintenance.request')].browse(int(editor_id)), osv_info.get('field', 'description_editor_ids'))]
        ))
    ...
    def editor_save(self, osv_info, editor_id, delete_items, **args):
            ...
            if str(index) + '-id' in args:
                ...
                if write_id:
                    ...
                    create_val = {
                        ...
                        'value': self._generate_image_path(osv_info.get('model', 'maintenance.request'), editor_id, item),
                        'index': index,
        ...
        write_model = request.env[osv_info.get('model', 'maintenance.request')]
        ...
        if request.jsonrequest.get('ids') and request.jsonrequest.get('ids') != ['undefined']:
            ...
            for image in request.jsonrequest.get('change_list'):
                ...
                if not int(image.get('index')):
                    request.env.cr.execute("""UPDATE maintenance_request jp SET image_path='%s' WHERE id ='%s'""" %
