...
class ProductImage(models.Model):
    ...
    image_path = fields.Char(u'维护申请图片地址')
    is_primary = fields.Boolean(u'是主图')
    order_sort = fields.Integer(u'图片顺序')
    template_id = fields.Many2one('maintenance.request', string=u'维护申请', index=True)

    _sql_constraints = [
        ('maintenance_request_sort_uniq', 'unique(template_id, order_sort)', "SKU 图片顺序 已经存在请核对后再进行添加 !"),
    ]
...
class ProductTemplate(models.Model):
    ...
    image_path = fields.Char(u'维护申请图片地址')
    introduction = fields.Text(u'维护申请详细描述')
...
class DescriptionEditorRow(models.Model):
    ...
    product_id = fields.Many2one('maintenance.request', u'维护申请')
